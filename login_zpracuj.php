...
if(isset($_POST['user-name'])){
  ...
  //$ip .= $_SERVER['REMOTE_ADDR'];  
    $query = mysql_query("SELECT * FROM `rules_users` WHERE `username` = '$name' and `password` = '$pass'") or die (mysql_error());
    // Vybereme uživatele se zadaným jménem a heslem
      $Vysledek = mysql_fetch_array($query);
        if($Vysledek['username']){ // pokud tato proměnná obsahuje data, bylo zadané správné jméno a heslo
          ...
          $qry = "UPDATE `rules_users` SET `ip-zaznam` = '$ip' WHERE `id` = {$Vysledek['id']}";
          $ret=mysql_query($qry);
          if (!$ret)
